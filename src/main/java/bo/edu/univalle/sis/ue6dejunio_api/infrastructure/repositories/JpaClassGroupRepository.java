...
public interface JpaClassGroupRepository extends JpaRepository<ClassGroupEntity, UUID> {
     ...
     *
     * <p>Carries the same graph as the two listings above, {@code course.grade} and {@code
     * course.parallel} included: every caller maps these through the adapter's {@code toDomain},
     * which reads the grade and the parallel to name the course. Left out of the graph they are two
     * more selects per row — the follow-up-per-row this graph exists to stop, just further down.
     */
    @EntityGraph(
            attributePaths = {
                "course",
