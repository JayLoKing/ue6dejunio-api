 ...
 * @param riskLevel the model's spelling, {@code RiesgoCritico} and not the enum constant — the same
 *     vocabulary the per-subject panel already reads.
 */
public record InstitutionRiskEntryResponse(
    ...
    public static InstitutionRiskEntryResponse from(InstitutionRiskEntry e) {
        return new InstitutionRiskEntryResponse(
                e.position(),
                e.predictionId(),
                e.studentId(),
                e.fullName(),
