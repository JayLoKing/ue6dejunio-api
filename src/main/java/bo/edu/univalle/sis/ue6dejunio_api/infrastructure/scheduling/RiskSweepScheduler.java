 ...
 *
 * <p>{@code fixedDelay} and not {@code fixedRate}: the delay is counted from the end of the
 * previous sweep, so a run that takes longer than the interval cannot have the next one start on
 * top of it. At a fixed rate, a slow model would stack ticks until several sweeps were asking about
 * the same subjects at once — and the queue rows they were draining would be deleted out from under
 * each other.
 */
@Component
...
public class RiskSweepScheduler {
     ...
     *
     * <p>The initial delay is not the same value as the interval and is not decoration: the model
     * is a separate process that loads TensorFlow on its first call, and a sweep firing while this
     * application is still coming up would spend that first minute waiting on a service that has
     * not finished starting either.
     */
    @Scheduled(
    ...
    public void sweep() {
        ...
        log.info(
                "Risk sweep: {} subject(s) taken, {} vector(s) considered, {} skipped, "
