 ...
 *
 * @param subjects    how many queued subjects were taken. Zero is the ordinary case — most ticks
 *                    find nothing, and a sweep that finds nothing must stay silent rather than
 *                    write a line every few minutes forever.
 * @param considered  student-subject vectors assembled
 * @param skipped     of those, the ones the model cannot be asked about yet: a dimension with no
 *                    mark, or a subject with nothing planned
 ...
 */
public record SweepSummary(int subjects, int considered, int skipped, int predicted,
                           int transitions) {
