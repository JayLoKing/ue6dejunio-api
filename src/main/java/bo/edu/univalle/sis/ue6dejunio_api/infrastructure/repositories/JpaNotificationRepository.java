...
public interface JpaNotificationRepository extends JpaRepository<NotificationEntity, UUID> {
    ...
    /**
     * Stamps delivery on what the inbox is about to hand over, in one statement rather than one
     * per row. Only the rows that were never delivered are touched, so a receiver polling every
     * thirty seconds writes once per notification and never again.
     *
     ...
     * the context carrying their old null, and the next read inside the same transaction is
     * served from there — the stamp appears to have been lost. The entities the caller is holding
     * are detached by the clear, which is harmless: their two associations are eager, so the
     * mapping that follows reads values already in hand and touches no context at all.
     */
    @Modifying(flushAutomatically = true, clearAutomatically = true)
    @Query("UPDATE NotificationEntity n SET n.deliveredAt = :now "
