...
public record SavePedagogicalReportRequest(
    ...
    // V17 says a length nobody measured would silently truncate a teacher's work — so nothing below
    // this point would refuse a body of any size. 4000 is what the PDC's prose already uses, and it
    // is several times the longest section II the school's own documents carry.
    @Size(max = 4000, message = "achievements no puede exceder 4000 caracteres") String achievements,
    @Size(max = 4000, message = "difficulties no puede exceder 4000 caracteres") String difficulties,
    // @NotNull on the element and not only @Valid on the list: @Valid cascades into the elements
    // that exist, so a literal null inside the array reaches no constraint at all and would arrive
    // at toDraft() as a null to dereference — a parseable body turned into a 500 where the sibling
    ...
    // classroom in this school comes near that. It is the same ceiling the service pages the roster
    ...
    List<@NotNull(message = "failingStudents no admite elementos nulos") FailingStudentNote>
        failingStudents
) {
    ...
    public PedagogicalReportDraft toDraft() {
        List<PedagogicalReportNote> notes = failingStudents == null
            ? null
            : failingStudents.stream()
                .map(n -> new PedagogicalReportNote(
                    n.idCourseEnrollment(), n.actions(), n.verificationSource()))
                .toList();
