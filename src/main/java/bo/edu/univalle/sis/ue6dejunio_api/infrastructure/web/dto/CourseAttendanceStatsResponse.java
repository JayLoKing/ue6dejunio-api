...
public record CourseAttendanceStatsResponse(
    ...
    public record Counts(
        long present, long absent, long late, long excused,
        long computableSessions, BigDecimal percentage
    ) {
        static Counts from(AttendanceCounts c) {
            return new Counts(c.present(), c.absent(), c.late(), c.excused(),
                c.computableSessions(), c.percentage());
        }
    }

    public record Month(
        int year, int month, long present, long absent, long late, long excused,
        long computableSessions, BigDecimal percentage
    ) {
        static Month from(MonthlyAttendance m) {
            AttendanceCounts c = m.counts();
            return new Month(m.year(), m.month(), c.present(), c.absent(), c.late(), c.excused(),
                c.computableSessions(), c.percentage());
        }
    }

    public record Trimester(
        int trimester, long present, long absent, long late, long excused,
        long computableSessions, BigDecimal percentage
    ) {
        static Trimester from(TrimesterAttendance t) {
            AttendanceCounts c = t.counts();
            return new Trimester(t.trimester(), c.present(), c.absent(), c.late(), c.excused(),
                c.computableSessions(), c.percentage());
        }
    }
