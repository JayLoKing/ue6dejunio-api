 ...
 * matrix, one under the trimester averages. They agree only while every area is graded in all three
 * trimesters, which is the state a closed year is in. {@code finalAverage} is the area one, because
 * that is the column the year-end sheet titles PROMEDIO FINAL and the one the ranking reads.
 * Reporting both would be handing the school two different answers to one question.
 *
 ...
 * @param subjects one entry per area of knowledge, ordered so the sheet's columns hold still
 *     between two loads.
 * @param trimester1Average null when the student has no graded row that trimester. Exactly what the
 *     trimester centralizer reports as their general average, so the annual and the trimester
 *     screens can never disagree about a student.
