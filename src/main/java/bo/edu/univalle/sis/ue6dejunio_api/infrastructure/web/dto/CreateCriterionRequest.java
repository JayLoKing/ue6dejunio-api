 ...
 * and the criterion is scored directly; include it and the criterion's score becomes the average of
 * the activity's items.
 */
public record CreateCriterionRequest(
    ...
    /**
     * The bound on {@code items} is structural, not cosmetic: every entry becomes one insert inside
     * a single transaction, so an unbounded list is an open door to a write of any size. Fifty is
     * far above any real activity — a trimester's notebook review runs to a handful.
     */
