 ...
 *
 * Percentage rule: percentage = present * 100 / (present + absent + late), rounded HALF_UP
 * to 1 decimal (0-100 scale). Excused is excluded from both numerator and denominator. Late counts as a miss
 * (denominator yes, numerator no). If computableSessions == 0, percentage is null (never 0,
 * never NaN).
 */
public record AttendanceCounts(
    ...
    public static AttendanceCounts of(long present, long absent, long late, long excused) {
        ...
        BigDecimal percentage = computableSessions == 0
            ? null
            : BigDecimal.valueOf(present)
                .multiply(BigDecimal.valueOf(100))
                .divide(BigDecimal.valueOf(computableSessions), 1, RoundingMode.HALF_UP);
