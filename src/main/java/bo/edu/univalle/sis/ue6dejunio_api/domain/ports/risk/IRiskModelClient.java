...
public interface IRiskModelClient {
     ...
     *              because the model rejects an incomplete one with a 422 and a rejected batch
     *              costs every vector in it
     * @return one score per vector, <b>in the same order as the input</b>. That positional pairing
     *         is the only thing tying a score back to its student, so an answer of a different
     *         length is an error, not a partial result.
