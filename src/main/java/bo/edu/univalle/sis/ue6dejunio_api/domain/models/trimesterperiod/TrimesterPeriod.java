...
/**
 * A Director-configured date range for one trimester of one academic year. Attendance rows carry no
 * trimester column; this is the single source of truth for mapping a date to a trimester (replaces
 * the previous hardcoded AcademicTrimesterCalendar month map).
 */
public record TrimesterPeriod(
        UUID id, Integer academicYearId, int trimester, LocalDate startDate, LocalDate endDate) {
