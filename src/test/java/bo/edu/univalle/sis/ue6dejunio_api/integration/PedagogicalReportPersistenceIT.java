 ...
 *
 * <p>A mocked port proves none of what is interesting here. Three things live below it and can only
 * break in the database: {@code UNIQUE (id_course, trimester)}, which is what makes a second save a
 * correction rather than a second opinion; the set semantics of the notes, where a student dropped
 * from the draft has to lose their paragraph; and whether a note that survives a save keeps the row
 * it already had, because a delete-and-reinsert would quietly reset when the teacher first wrote
 * it.
 */
class PedagogicalReportPersistenceIT extends AbstractIntegrationTest {

    @Autowired private IPedagogicalReportDomain reports;
    ...
    void save_withNothingWrittenYet_isStillADocument() {
        PedagogicalReport stored =
                reports.save(courseId, 2, new PedagogicalReportDraft(null, null, List.of()));
    ...
    void save_twice_correctsTheSameRowInsteadOfFilingASecond() {
        UUID firstId =
                reports.save(
                                courseId,
                                1,
                                new PedagogicalReportDraft("Primer borrador", null, List.of()))
                        .id();

        PedagogicalReport corrected =
                reports.save(
                        courseId,
                        1,
                        new PedagogicalReportDraft(
                                "Version final", "Dificultades observadas", List.of()));
    ...
    void save_withoutANoteThatWasThere_deletesThatStudentsParagraph() {
        ...
        assertThat(corrected.notes())
                .containsExactly(note(brunoEnrollment, "Acciones para Bruno", "Fuente Bruno"));
        assertThat(
                        jdbc.queryForObject(
    ...
    void save_withNullNotes_leavesEveryStoredParagraphWhereItWas() {
        ...
        PedagogicalReport prose =
                reports.save(
                        courseId, 1, new PedagogicalReportDraft("Sólo los logros", null, null));
    ...
    void save_aNoteThatSurvives_keepsTheRowItAlreadyHad() {
        reports.save(
                ...
                1,
                new PedagogicalReportDraft(
                        null, null, List.of(note(brunoEnrollment, "Primera version", "Cuaderno"))));
        UUID rowId =
                jdbc.queryForObject(
     ...
     * read-then-insert, the save that never saw it would insert its own and fail {@code
     * uq_pedagogical_report} — a 409 on the endpoint whose whole promise is that saving twice
     * leaves the same document. With the upsert, it finds the conflict and corrects the row.
     */
    @Test
    void save_reportRowAlreadyThere_correctsItInsteadOfCollidingWithTheUniqueConstraint() {
        jdbc.update(
                "INSERT INTO pedagogical_reports (id_course, trimester, achievements) "
                        + "VALUES (?,?,?)",
                courseId,
                1,
                "Escrito por otra transaccion");

        PedagogicalReport saved =
                reports.save(
                        courseId, 1, new PedagogicalReportDraft("Mi version", null, List.of()));
    ...
    void save_noteRowAlreadyThere_correctsItInsteadOfCollidingWithTheUniqueConstraint() {
        UUID reportId =
                reports.save(courseId, 1, new PedagogicalReportDraft(null, null, List.of())).id();
        jdbc.update(
        ...
        assertThat(saved.notes()).containsExactly(note(brunoEnrollment, "Mi version", "Cuaderno"));
        assertThat(
                        jdbc.queryForObject(
                                "SELECT COUNT(*) FROM pedagogical_report_failures", Integer.class))
                .isEqualTo(1);
    ...
    void save_onlyTouchesTheTrimesterItNames() {
        ...
        reports.save(
                courseId, 2, new PedagogicalReportDraft("Logros del segundo", null, List.of()));
        ...
        assertThat(first.get().notes())
                .containsExactly(note(anaEnrollment, "Acciones del primero", null));
    }
}
