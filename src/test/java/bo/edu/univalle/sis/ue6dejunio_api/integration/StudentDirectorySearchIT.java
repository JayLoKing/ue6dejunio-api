...
/**
 * Spec: RF31 - GET /api/students/search. Blank q routes to the no-filter list query; non-blank q
 * routes to the LIKE query; grade/parallel/level are derived from the active (Effective) course
 * enrollment; Teacher is auto-scoped to their own homeroom course and never gets 403.
 */
class StudentDirectorySearchIT extends AbstractIntegrationTest {
    ...
    /**
     * The one that made the enrolment join worth rewriting: a withdrawal closes the enrolments too,
     * so joined on 'Effective' alone every withdrawn student came back with no course and this
     * filter excluded all of them.
     */
    @Test
    void withdrawnStudent_keepsTheGradeTheyWereIn() throws Exception {
        ...
        Integer gradeId =
                jdbc.queryForObject(
                        "SELECT id_grade FROM courses WHERE id_course = ?",
                        Integer.class,
                        homeroomCourse);
    ...
    void teacherPinnedToTheirCourse_isNotNarrowedByTheCurrentGestion() throws Exception {
        ...
        mvc.perform(
                        get("/api/students/search")
                                .header(
                                        "Authorization",
                                        "Bearer " + tokenFor(pastTeacher, "Teacher")))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.content.length()").value(1))
                .andExpect(jsonPath("$.content[0].id").value(pastStudent.toString()));
    }
    ...
    private void withdraw(UUID studentId) throws Exception {
        mvc.perform(
                        post("/api/students/{id}/withdraw", studentId)
