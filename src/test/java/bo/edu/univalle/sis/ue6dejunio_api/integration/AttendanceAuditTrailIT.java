 ...
 * Because both write paths are upserts, a correction must move updated_by/updated_at while
 * leaving created_by/created_at on the original author.
 */
class AttendanceAuditTrailIT extends AbstractIntegrationTest {
    ...
    void correction_movesUpdatedAuthor_andKeepsOriginalAuthor() throws Exception {
        ...
        Integer rows = jdbc.queryForObject(
            "SELECT COUNT(*) FROM attendance WHERE id_course_enrollment = ?", Integer.class, enrollment);
        assertThat(rows).isEqualTo(1);
    }

    private void postDaily(UUID userId, String role, String status) throws Exception {
        String body = json.writeValueAsString(new DailyBody(enrollment, editableSchoolDay(), status));
    ...
    private Map<String, Object> auditRow() {
        return jdbc.queryForMap(
            "SELECT status, created_by, updated_by, created_at, updated_at FROM attendance "
                + "WHERE id_course_enrollment = ? AND id_class_group IS NULL", enrollment);
    }
