...
class RiskInstitutionIT extends AbstractIntegrationTest {
    ...
    private static final RiskFeatures FEATURES = new RiskFeatures(
        ...
        List.of(new BigDecimal("28")), List.of(new BigDecimal("4")),
    ...
    void seedSchool() {
        ...
        riskPredictions.upsertAll(List.of(
            // Two subjects for Ana: the list has to pick the worse of them and count her once.
    ...
    private NewRiskPrediction prediction(UUID student, UUID classGroup, RiskLevel level, String pFail) {
        return new NewRiskPrediction(student, classGroup, 1, level,
            new BigDecimal(pFail), new BigDecimal("0.0100"), FEATURES,
            LocalDateTime.of(2026, 4, 10, 8, 0));
    }

    /**
     * The point of the whole-school scope: the worst of the building, not the worst of one room. The
     * worst prediction sits in course B, so a list that opens with course A's is reading one course.
     */
    @Test
    void institutionRisk_reachesAcrossEveryCourseAndCountsAStudentOnce() throws Exception {
        String body = mvc.perform(get("/api/risk/institution")
