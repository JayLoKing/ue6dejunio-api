 ...
 * introduced to fix the centralizer/courseAttendance N+1. Confirms the batched path produces output
 * identical to the previous per-enrollment lookup path, with no cross-student bleed and unchanged
 * empty-scores shape. This is the DB-independent safety net; the byte-identical golden JSON tests
 * and the Hibernate Statistics query-count test additionally require Testcontainers Postgres and
 * are gated by {@code disabledWithoutDocker}.
 */
...
class GradebookServiceGroupingTest {
    ...
    void courseAttendance_groupsBatchedAttendancePerEnrollment_noBleed() {
        ...
        Attendance attA1 =
                new Attendance(
                        UUID.randomUUID(), enrollmentA, null, LocalDate.of(2026, 3, 1), "Present");
        Attendance attA2 =
                new Attendance(
                        UUID.randomUUID(), enrollmentA, null, LocalDate.of(2026, 3, 2), "Absent");
        Attendance attB1 =
                new Attendance(
                        UUID.randomUUID(), enrollmentB, null, LocalDate.of(2026, 3, 1), "Present");
        ...
        when(enrollmentDomain.courseStudentById(enrollmentA))
                .thenReturn(
                        java.util.Optional.of(
                                courseStudent(enrollmentA, studentA, "Ana", "Perez")));
        when(scoreDomain.findByCourseEnrollment(enrollmentA))
                .thenReturn(
                        List.of(
                                score(
                                        enrollmentA,
