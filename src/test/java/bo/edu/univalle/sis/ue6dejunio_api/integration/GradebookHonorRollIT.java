 ...
 * itself wrote. What only a database can answer is whether the podium is ordered by the same {@code
 * total_score} the school's sheets print — a generated column this code never writes — and whether
 * the whole-school scope really reaches across courses instead of reading one.
 */
class GradebookHonorRollIT extends AbstractIntegrationTest {
    ...
    void seed() {
        ...
        gradeOneArea(
                ...
                new BigDecimal("5"));
        gradeOneArea(
                seedEnrollment(seedStudent("Carla", "Rojas"), courseA),
                langA,
                new BigDecimal("5"),
                new BigDecimal("20"),
                new BigDecimal("20"),
        ...
        gradeOneArea(
                seedEnrollment(seedStudent("Dario", "Soto"), courseB),
                langB,
                new BigDecimal("10"),
                new BigDecimal("45"),
                new BigDecimal("40"),
    ...
    void institutionHonorRoll_reachesAcrossEveryCourseOfTheSchool() throws Exception {
        String body =
                mvc.perform(
                                get("/api/gradebook/honor-roll/institution")
                                        .header(
                                                "Authorization",
                                                "Bearer " + tokenFor(director, "Director"))
                                        .param(
