...
/**
 * Spec: Daily attendance homeroom ownership guard, dailyBatch atomicity, attendance read guard, and
 * session() regression (unchanged @authz.canWriteClassGroup guard).
 */
class AttendanceAuthorizationIT extends AbstractIntegrationTest {
    ...
    void nonHomeroomTeacher_daily_forbidden() throws Exception {
        String body =
                json.writeValueAsString(
                        new DailyBody(enrollmentInA, editableSchoolDay(), "Present"));
    ...
    void homeroomTeacher_daily_ok() throws Exception {
        ...
        mvc.perform(
                        post("/api/attendance/daily")
                                .header(
                                        "Authorization",
                                        "Bearer " + tokenFor(homeroomTeacherA, "Teacher"))
                                .contentType(MediaType.APPLICATION_JSON)
                                .content(body))
    ...
    void mixedOwnershipBatch_forbidden_nothingPersisted() throws Exception {
        String body =
                json.writeValueAsString(
                        new DailyBatchBody(
                                editableSchoolDay(),
                                List.of(
                                        new Mark(enrollmentInA, "Present"),
                                        new Mark(enrollmentInB, "Present"))));

        mvc.perform(
                        post("/api/attendance/daily/batch")
                                .header(
                                        "Authorization",
                                        "Bearer " + tokenFor(homeroomTeacherA, "Teacher"))
                                .contentType(MediaType.APPLICATION_JSON)
        ...
        Integer count =
                jdbc.queryForObject(
                        "SELECT COUNT(*) FROM attendance WHERE id_course_enrollment IN (?, ?)",
                        Integer.class,
                        enrollmentInA,
                        enrollmentInB);
    ...
    void director_dailyAndBatch_ok() throws Exception {
        String dailyBody =
                json.writeValueAsString(
                        new DailyBody(enrollmentInB, editableSchoolDay(), "Present"));
        mvc.perform(
                        post("/api/attendance/daily")
    ...
    void session_regression_nonOwnerForbidden_ownerAndDirectorOk() throws Exception {
        String body =
                json.writeValueAsString(
                        new SessionBody(
                                enrollmentInA, classGroupA, editableSchoolDay(), "Present"));
     ...
     * These tests assert authorization, not the date window, so the date must never be the reason a
     * request is rejected — on a weekend run this falls back to that week's Friday, which the
    ...
    private record Mark(
            @JsonProperty("id_course_enrollment") UUID courseEnrollmentId, String status) {}

    private record DailyBatchBody(
            @JsonFormat(pattern = "yyyy-MM-dd") LocalDate date, List<Mark> records) {}
