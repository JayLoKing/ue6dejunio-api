...
class CourseOverviewIT extends AbstractIntegrationTest {
    ...
    private void seedStudentWithScore(UUID courseId, UUID classGroupId, BigDecimal total) {
        ...
        jdbc.update(
                ...
                // doing<=40, deciding<=5). Pin three of them at their cap and let "doing" absorb
                // the
                ...
                new BigDecimal("10"),
                new BigDecimal("45"),
                total.subtract(new BigDecimal("60")),
                new BigDecimal("5"));
    }
    ...
    void fullComposite_returned_forSeededCourse() throws Exception {
        ...
        assertThat(root.get("course").get("homeroomTeacherId").asText())
                .isEqualTo(homeroomTeacher.toString());
        assertThat(root.get("classGroups")).hasSize(1);
        assertThat(root.get("classGroups").get(0).get("subjectName").asText())
    ...
    void studentGradeLoading_isN1Safe_queryCountConstant() throws Exception {
        ...
        stats.clear();
        mvc.perform(
                        get("/api/courses/{id}/overview", courseLarge)
                                .header("Authorization", "Bearer " + tokenFor(director, "Director"))
                                .param("trimester", "1")
                                .param("offset", "1")
                                .param("limit", "200"))
        ...
        assertThat(countForFiftyStudents)
                .as(
                        "grade-fetching query count must not scale with student count (batch centralizer path)")
    ...
    void invalidCourseId_returns404() throws Exception {
        mvc.perform(
                        get("/api/courses/{id}/overview", UUID.randomUUID())
                                .header("Authorization", "Bearer " + tokenFor(director, "Director"))
                                .param("trimester", "1"))
    ...
    void unauthenticated_rejected_401() throws Exception {
        mvc.perform(get("/api/courses/{id}/overview", courseA).param("trimester", "1"))
