...
/**
 * Spec: Score write/read ownership guard + Resolver not-found handling.
 * Endpoints: AssessmentScoreController.setScore/delete/byEvent/byCourseEnrollment.
 */
class AssessmentScoreAuthorizationIT extends AbstractIntegrationTest {
    ...
    void nonOwnerTeacher_setScore_forbidden_andNothingPersisted() throws Exception {
        ...
        mvc.perform(post("/api/assessment-scores")
        ...
        Integer count = jdbc.queryForObject(
            "SELECT COUNT(*) FROM assessment_scores WHERE id_assessment_event = ?", Integer.class, eventId);
    ...
    void director_deletesScore_regardlessOfOwnership() throws Exception {
        jdbc.update(
            "INSERT INTO assessment_scores (id_assessment_score, id_course_enrollment, id_assessment_event, score) "
                + "VALUES (?,?,?,?)",
            UUID.fromString("00000000-0000-0000-0000-000000000001"), studentEnrollment, eventId, 10.0);
    ...
    void nonOwnerTeacher_byEvent_forbidden() throws Exception {
        mvc.perform(get("/api/assessment-scores/event/{eventId}", eventId)
                .header("Authorization", "Bearer " + tokenFor(teacherB, "Teacher")))
            .andExpect(status().isForbidden());
    }
    ...
    void unknownEventId_setScore_deniesNotCrashes() throws Exception {
        String body = json.writeValueAsString(new SetScoreBody(studentEnrollment, UUID.randomUUID(), "40"));
    ...
    private record SetScoreBody(
        @com.fasterxml.jackson.annotation.JsonProperty("id_course_enrollment") UUID courseEnrollmentId,
