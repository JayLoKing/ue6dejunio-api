...
class KnowledgeAreaIT extends AbstractIntegrationTest {
    ...
    private Integer createArea(String name) throws Exception {
        String body = mvc.perform(post("/api/knowledge-areas")
    ...
    void create_thenReadItBack() throws Exception {
        ...
        mvc.perform(get("/api/knowledge-areas/{id}", id)
                .header("Authorization", asDirector()))
            .andExpect(status().isOk())
    ...
    void create_withoutAnOrder_landsAfterTheOnesAlreadyThere() throws Exception {
        Integer maxBefore = jdbc.queryForObject(
            "SELECT COALESCE(MAX(display_order), 0) FROM knowledge_areas", Integer.class);

        Integer id = createArea("Area sin orden");

        Integer order = jdbc.queryForObject(
            "SELECT display_order FROM knowledge_areas WHERE id_area = ?", Integer.class, id);
    ...
    void create_duplicateName_returns409() throws Exception {
        ...
        mvc.perform(post("/api/knowledge-areas")
                .header("Authorization", asDirector())
                .contentType(MediaType.APPLICATION_JSON)
    ...
    void update_renamesTheArea() throws Exception {
        ...
        mvc.perform(put("/api/knowledge-areas/{id}", id)
                .header("Authorization", asDirector())
                .contentType(MediaType.APPLICATION_JSON)
    ...
    void delete_emptyArea_returns204() throws Exception {
        ...
        mvc.perform(delete("/api/knowledge-areas/{id}", id)
                .header("Authorization", asDirector()))
    ...
    void delete_areaWithSubjects_returns409() throws Exception {
        ...
        jdbc.update("INSERT INTO subjects (id_subject, name, id_area, is_technical, is_active) "
                + "VALUES (?,?,?,false,true)",
            UUID.randomUUID(), "Materia de " + id, id);
        ...
        assertThat(jdbc.queryForObject(
            "SELECT COUNT(*) FROM knowledge_areas WHERE id_area = ?", Integer.class, id))
            .isEqualTo(1);
    }
    ...
    void list_comesInPrintingOrder() throws Exception {
        String body = mvc.perform(get("/api/knowledge-areas").param("limit", "50")
                .header("Authorization", asDirector()))
            .andExpect(status().isOk())
            .andReturn().getResponse().getContentAsString();
        ...
        assertThat(orders).isSortedAccordingTo(
            (a, b) -> Integer.compare(a.asInt(), b.asInt()));
    }

    @Test
    void secretary_isRefused() throws Exception {
        mvc.perform(get("/api/knowledge-areas")
    ...
    void teacher_isRefused() throws Exception {
        mvc.perform(get("/api/knowledge-areas")
                .header("Authorization", "Bearer " + tokenFor(
                    seedUser("Teacher", false), "Teacher")))
            .andExpect(status().isForbidden());
    }
}
