 ...
 * technical teacher load their subject roster and save the whole group in one transaction. Also
 * pins the two boundaries: the secretariat never writes, and never reaches grades.
 */
class AttendanceSecretaryAndSessionBatchIT extends AbstractIntegrationTest {
    ...
    void secretary_readsEnrollmentAttendance() throws Exception {
        mvc.perform(
                        get("/api/attendance")
                                .param("id_course_enrollment", enrollmentA.toString())
                                .header(
                                        "Authorization",
                                        "Bearer " + tokenFor(secretary, "Secretary")))
                .andExpect(status().isOk());
    ...
    void secretary_readsAttendanceStats() throws Exception {
        mvc.perform(
                        get("/api/courses/" + course + "/attendance-stats")
                                .header(
                                        "Authorization",
                                        "Bearer " + tokenFor(secretary, "Secretary")))
                .andExpect(status().isOk());
    ...
    void secretary_cannotWriteAttendance() throws Exception {
        String body =
                json.writeValueAsString(new DailyBody(enrollmentA, editableSchoolDay(), "Present"));
    ...
    void secretary_cannotWithdrawStudent() throws Exception {
        ...
        mvc.perform(
                        post("/api/students/" + UUID.randomUUID() + "/withdraw")
                                .header(
                                        "Authorization",
                                        "Bearer " + tokenFor(secretary, "Secretary"))
                                .contentType(MediaType.APPLICATION_JSON)
    ...
    void technicalTeacher_loadsSubjectRoster() throws Exception {
        mvc.perform(
                        get("/api/gradebook/attendance/session")
    ...
    void technicalTeacher_savesWholeGroupInOneCall() throws Exception {
        String body =
                json.writeValueAsString(
                        new SessionBatchBody(
                                classGroup,
                                editableSchoolDay(),
                                List.of(
                                        new Mark(enrollmentA, "Present"),
                                        new Mark(enrollmentB, "Absent"))));

        mvc.perform(
                        post("/api/attendance/session/batch")
                                .header(
                                        "Authorization",
                                        "Bearer " + tokenFor(technicalTeacher, "Teacher"))
                                .contentType(MediaType.APPLICATION_JSON)
        ...
        Integer sessionRows =
                jdbc.queryForObject(
                        "SELECT COUNT(*) FROM attendance WHERE id_class_group = ?",
                        Integer.class,
                        classGroup);
        ...
        Integer dailyRows =
                jdbc.queryForObject(
                        "SELECT COUNT(*) FROM attendance WHERE id_class_group IS NULL",
                        Integer.class);
    ...
    void sessionBatch_withForeignStudent_isRejectedAndPersistsNothing() throws Exception {
        String body =
                json.writeValueAsString(
                        new SessionBatchBody(
                                ...
                                List.of(
                                        new Mark(enrollmentA, "Present"),
                                        new Mark(foreignEnrollment, "Present"))));
        ...
        Integer rows = jdbc.queryForObject("SELECT COUNT(*) FROM attendance", Integer.class);
