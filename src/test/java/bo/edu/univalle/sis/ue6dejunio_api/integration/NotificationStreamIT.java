 ...
 * registry: it is the route being authenticated, the response being an event stream at all, and
 * the listener running after the send commits. A mocked registry answers all three the same way
...
class NotificationStreamIT extends AbstractIntegrationTest {
    ...
    private MvcResult openStream(UUID reader, String role) throws Exception {
        return mvc.perform(get("/api/notifications/stream")
    ...
    /**
     * The first event is what flushes the response headers. Without it the browser sits on an
     * open request it cannot tell apart from one that never connected, and the client's fallback
     * poll would be switched off on the strength of a connection that is not there.
     */
    @Test
    ...
    /**
     * The whole chain, end to end: the Director writes, the send commits, and the teacher holding
     * a stream open is told to go and read it.
     *
