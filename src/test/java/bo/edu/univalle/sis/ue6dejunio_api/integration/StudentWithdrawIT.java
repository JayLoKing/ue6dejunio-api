...
/**
 * Spec: RF32 - POST /api/students/{id}/withdraw. Flips students.status to Withdrawn with the given
 * reason, cascades to the student's active course enrollment, drops from the default active search,
 * and keeps history readable. Repeat call conflicts (409); invalid reason (400); unknown student
 * (404).
 */
class StudentWithdrawIT extends AbstractIntegrationTest {
    ...
    void withdraw_activeStudent_returns204AndFlipsStatusAndEnrollment() throws Exception {
        ...
        String reason =
                jdbc.queryForObject(
                        "SELECT status_reason FROM students WHERE id_student = ?",
                        String.class,
                        student);
        String enrollmentStatus =
                jdbc.queryForObject(
                        "SELECT status FROM course_enrollments WHERE id_student = ?",
                        String.class,
                        student);
    ...
    void withdraw_nonexistentStudent_returns404() throws Exception {
        mvc.perform(
                        post("/api/students/{id}/withdraw", UUID.randomUUID())
                                .header("Authorization", "Bearer " + tokenFor(director, "Director"))
                                .contentType("application/json")
                                .content("{\"reason\":\"Otro\",\"note\":\"Se mudo.\"}"))
    ...
    void withdraw_recordsTheWordsTheAuthorAndTheMoment() throws Exception {
        mvc.perform(
                        post("/api/students/{id}/withdraw", student)
                                ...
                                .contentType("application/json")
                                .content(
                                        "{\"reason\":\"Otro\",\"note\":\"Se mudó a Santa Cruz.\"}"))
                .andExpect(status().isNoContent());
        ...
        assertThat(
                        jdbc.queryForObject(
                                "SELECT status_note FROM students WHERE id_student = ?",
                                String.class,
                                student))
        ...
        assertThat(
                        jdbc.queryForObject(
                                "SELECT status_changed_at FROM students WHERE id_student = ?",
                                Object.class,
                                student))
                .isNotNull();
    }
    ...
    void withdraw_tellsTheTeacherWhoseCourseTheStudentWasIn() throws Exception {
        mvc.perform(
                        post("/api/students/{id}/withdraw", student)
                                ...
                                .contentType("application/json")
                                .content(
                                        "{\"reason\":\"Transferencia\",\"note\":\"A la U.E. San Martín.\"}"))
                .andExpect(status().isNoContent());
