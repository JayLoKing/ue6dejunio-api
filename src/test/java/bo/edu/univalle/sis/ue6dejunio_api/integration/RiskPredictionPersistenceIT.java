...
class RiskPredictionPersistenceIT extends AbstractIntegrationTest {
    ...
    private NewRiskPrediction prediction(
            UUID student, UUID classGroup, RiskLevel level, String pFail, String pOutstanding) {
    ...
    void upsertAll_firstRun_writesEveryFieldBackIntact() {
        List<IRiskPredictionDomain.UpsertResult> results =
    ...
    void upsertAll_featureVector_survivesTheJsonbColumn() {
        riskPredictions.upsertAll(
                List.of(prediction(ana, mathGroup, RiskLevel.EN_RIESGO, "0.1200", "0.0500")));
        ...
        assertThat(named)
                ...
                .satisfies(
                        risk -> {
                            ...
                            assertThat(risk.prediction().riskLevel())
                                    .isEqualTo(RiskLevel.RIESGO_CRITICO);
                        });
    }
    ...
    void upsertAll_sameStudentTwoSubjects_keepsBothRows() {
        riskPredictions.upsertAll(
                List.of(
                        ...
                        prediction(
                                ana, languageGroup, RiskLevel.SOBRESALIENTE, "0.0100", "0.9000")));

        Integer rows =
                jdbc.queryForObject(
                        "SELECT COUNT(*) FROM risk_predictions WHERE id_student = ?",
                        Integer.class,
                        ana);
    ...
    void upsertAll_secondRun_correctsTheStandingRowAndReportsWhatItHeld() {
        riskPredictions.upsertAll(
                List.of(prediction(ana, mathGroup, RiskLevel.EN_RIESGO, "0.2000", "0.0300")));

        List<IRiskPredictionDomain.UpsertResult> second =
                riskPredictions.upsertAll(
                        List.of(
                                prediction(
                                        ana,
    ...
    void upsertAll_sameLevelAgain_reportsNoChange() {
        ...
        List<IRiskPredictionDomain.UpsertResult> second =
                riskPredictions.upsertAll(
                        List.of(
                                prediction(
                                        ana, mathGroup, RiskLevel.EN_RIESGO, "0.3100", "0.0200")));
    ...
    void upsertAll_aWholeRun_returnsResultsInTheOrderItWasGiven() {
        ...
        assertThat(results)
                .extracting(r -> r.stored().studentId())
                .containsExactly(bruno, ana, ana);
        assertThat(results)
                .extracting(r -> r.stored().classGroupId())
    ...
    void byClassGroupAndTrimester_ordersTheFailingStudentsFirst() {
        ...
        riskPredictions.upsertAll(
                List.of(
                        ...
                        prediction(
                                carla, mathGroup, RiskLevel.RIESGO_CRITICO, "0.8100", "0.0050")));

        List<StudentRisk> list = riskPredictions.byClassGroupAndTrimester(mathGroup, 1);

        assertThat(list)
                .extracting(r -> r.prediction().riskLevel())
                .containsExactly(
                        RiskLevel.RIESGO_CRITICO, RiskLevel.EN_RIESGO, RiskLevel.SOBRESALIENTE);
        assertThat(list)
                .extracting(StudentRisk::studentFullName)
                .containsExactly("Castro Carla", "Bermudez Bruno", "Alvarez Ana");
        assertThat(list).extracting(StudentRisk::subjectName).containsOnly("Matematicas");
    }
    ...
    void byCourseAndTrimester_spansEverySubjectAndNamesThem() {
        ...
        assertThat(list)
                .extracting(StudentRisk::subjectName)
    ...
    void byStudent_returnsEverySubjectTheStudentSits() {
        riskPredictions.upsertAll(
                List.of(
                        ...
                        prediction(
                                bruno, mathGroup, RiskLevel.RIESGO_CRITICO, "0.9000", "0.0010")));
    ...
    void markAttended_recordsThatSomebodyActedOnIt() {
        ...
        assertThat(riskPredictions.findById(id))
                .get()
                .extracting(RiskPrediction::attended)
                .isEqualTo(true);
    }
    ...
    void claimForNotification_secondTimeSameDay_isRefused() {
        ...
        Set<UUID> second =
                riskPredictions.claimForNotification(List.of(id), morning.plusHours(3), startOfDay);
    ...
    void claimForNotification_grantsOnlyTheRowsThatHaveNotSpokenToday() {
        List<IRiskPredictionDomain.UpsertResult> rows =
                riskPredictions.upsertAll(
                        List.of(
                                prediction(
                                        ana,
